...
async def test_create_and_delete_qr_code():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        # Only attempt deletion if creation was successful
        if create_response.status_code == 201:
            qr_data = create_response.json()
            filename = qr_data["filename"]
            delete_response = await ac.delete(f"/qr-codes/{filename}", headers=headers)
            assert delete_response.status_code == 200
            assert delete_response.json()["message"] == "QR code deleted successfully"